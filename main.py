...
def make_nearest_towns_list(my_postcode, desired_max_distance):
    ...
    for element in res:
        actual_distance=haversine((my_location_latitude, my_location_longitude), (element['lat'], element['lon']), unit=Unit.MILES)
        if int(actual_distance) < int(desired_max_distance):
            list_of_dictionaries_of_nearest_towns.append({'name':element['name'],'postcode':element['postcode'],'latitude':element['lat']})
